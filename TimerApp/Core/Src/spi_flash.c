...
uint8_t W25QXX_BUFFER[SPI_FLASH_WRITE_BUFFER_LEN];
...
#if SPI_FLASH_DMA_ENABLE
  g_RdOver = 0;
  HAL_SPI_Receive_DMA(&hspi3, pBuffer, NumByteToRead);
  //HAL_SPI_Receive_IT(&hspi3, pBuffer, NumByteToRead);

  uint32_t tick = HAL_GetTick();
  uint32_t timeout = tick + NumByteToRead * 10;
  while (g_RdOver == 0 && HAL_GetTick() < timeout);
#else
  if (HAL_SPI_Receive(&hspi3, pBuffer, NumByteToRead, NumByteToRead * 10) != HAL_OK)
  {
	Error_Handler();
  }
#endif
...
#if SPI_FLASH_DMA_ENABLE
  g_WrOver = 0;
  HAL_SPI_Transmit_DMA(&hspi3, pBuffer, NumByteToWrite);

  uint32_t tick = HAL_GetTick();
  uint32_t timeout = tick + NumByteToWrite * 20;
  while (g_WrOver == 0 && HAL_GetTick() < timeout);
#else 
	HAL_SPI_Transmit(&hspi3, pBuffer, NumByteToWrite, NumByteToWrite * 5);
#endif
...
void W25QXX_Write(uint8_t* pBuffer, uint32_t WriteAddr, uint16_t NumByteToWrite)
{
  ...
  while(1)
  {
    W25QXX_Read(pBuff,secpos*4096,4096);//读出整个扇区的内容
    for(i=0;i<secremain;i++)//校验数据
    {
        if(pBuff[secoff+i]!=0XFF)break;//需要擦除
    }
    if(i<secremain)//需要擦除
    {
      ...
      W25QXX_Write_NoCheck(pBuff,secpos*4096,4096);//写入整个扇区
    }
	else
	{
